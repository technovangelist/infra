...
func TestSecretProvider_UnmarshalMap_IntegrationWithDecodeHook_FullConfig(t *testing.T) {
	content := `
secrets:
...
func TestSecretProvider_UnmarshalMap(t *testing.T) {
	...
	run := func(t *testing.T, tc testCase) {
		sp := SecretProvider{}
		err := sp.UnmarshalMap(tc.source, decodeConfig)
		if tc.expectedErr != "" {
		...
		assert.NilError(t, err)
		assert.DeepEqual(t, sp, tc.expected)
	}
	...
	testCases := []testCase{
		...
		{
			name:        "unknown kind",
			source:      rawConfig{"kind": "what"},
			expectedErr: `unknown secret provider kind "what", expected one of`,
		},
		{
